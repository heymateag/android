...
public class StickersArchiveAlert extends AlertDialog.Builder {
    ...
    public StickersArchiveAlert(Context context, BaseFragment baseFragment, ArrayList<TLRPC.StickerSetCovered> sets) {
        ...
        if (set.set.masks) {
            ...
            setTitle(LocaleController.getString("ArchivedMasksAlertTitle", works.heymate.beta.R.string.ArchivedMasksAlertTitle));
        } else {
            currentType = MediaDataController.TYPE_IMAGE;
            setTitle(LocaleController.getString("ArchivedStickersAlertTitle", works.heymate.beta.R.string.ArchivedStickersAlertTitle));
        }
        ...
        if (set.set.masks) {
            textView.setText(LocaleController.getString("ArchivedMasksAlertInfo", works.heymate.beta.R.string.ArchivedMasksAlertInfo));
        } else {
            textView.setText(LocaleController.getString("ArchivedStickersAlertInfo", works.heymate.beta.R.string.ArchivedStickersAlertInfo));
        }
        ...
        setNegativeButton(LocaleController.getString("Close", works.heymate.beta.R.string.Close), (dialog, which) -> dialog.dismiss());
        if (parentFragment != null) {
            setPositiveButton(LocaleController.getString("Settings", works.heymate.beta.R.string.Settings), (dialog, which) -> {

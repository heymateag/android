...
public class VoIPHelper {
	...
	private static void doInitiateCall(TLRPC.User user, TLRPC.Chat chat, String hash, TLRPC.InputPeer peer, boolean hasFewPeers, boolean videoCall, boolean canVideoCall, boolean createCall, Activity activity, BaseFragment fragment, AccountInstance accountInstance, boolean checkJoiner, boolean checkAnonymous) {
		...
		if (checkAnonymous && !hasFewPeers && peer instanceof TLRPC.TL_inputPeerUser && ChatObject.shouldSendAnonymously(chat) && (!ChatObject.isChannel(chat) || chat.megagroup)) {
			new AlertDialog.Builder(activity)
					.setTitle(LocaleController.getString("VoipGroupVoiceChat", works.heymate.beta.R.string.VoipGroupVoiceChat))
					.setMessage(LocaleController.getString("VoipGroupJoinAnonymouseAlert", works.heymate.beta.R.string.VoipGroupJoinAnonymouseAlert))
					.setPositiveButton(LocaleController.getString("VoipChatJoin", works.heymate.beta.R.string.VoipChatJoin), (dialog, which) -> doInitiateCall(user, chat, hash, peer, false, videoCall, canVideoCall, createCall, activity, fragment, accountInstance, false, false))
					.setNegativeButton(LocaleController.getString("Cancel", works.heymate.beta.R.string.Cancel), null)
					.show();
			return;
		}
    ...
    public static void showGroupCallAlert(BaseFragment fragment, TLRPC.Chat currentChat, TLRPC.InputPeer peer, boolean recreate, AccountInstance accountInstance) {
		...
		JoinCallAlert.checkFewUsers(fragment.getParentActivity(), -currentChat.id, accountInstance, param -> {
			/*if (param) {
				...
				builder.setTitle(LocaleController.getString("StartVoipChatTitle", works.heymate.beta.R.string.StartVoipChatTitle));
				if (recreate) {
					builder.setMessage(LocaleController.getString("VoipGroupEndedStartNew", works.heymate.beta.R.string.VoipGroupEndedStartNew));
				} else {
					if (ChatObject.isChannel(currentChat) && !currentChat.megagroup) {
						builder.setMessage(LocaleController.getString("StartVoipChannelAlertText", works.heymate.beta.R.string.StartVoipChannelAlertText));
					} else {
						builder.setMessage(LocaleController.getString("StartVoipChatAlertText", works.heymate.beta.R.string.StartVoipChatAlertText));
					}
				}
